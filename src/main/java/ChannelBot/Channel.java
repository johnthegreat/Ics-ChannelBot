...
public class Channel implements Comparable<Channel> {
	public static ChannelChangedEventListener channelChangedEventListener = new ChannelChangedEventListener() {
		@Override
		public void run() {
			final ChannelBot channelBot = this.getChannel().channelBot;
			try {
				channelBot.getDatabaseProviderRepository().getChannelProvider().updateChannel(getChannel());
				final List<String> channelMembers = getChannel().getMembers();
				for (final String username : channelMembers) {
					final User user = channelBot.getUserService().getUser(username);
					if (user == null) {
						System.err.printf("User could not be found (so was not persisted): %s%n", username);
						return;
					}
					channelBot.getDatabaseProviderRepository().getChannelUserProvider().createOrUpdateChannelUser(getChannel(), user);
				}
	...
	public Channel(final ChannelBot channelBot) {
		setChannelBot(channelBot);
		setMembers(new ArrayList<>());
		moderators = new ArrayList<>();
		historyTime = 1000 * 60;
		if (channelBot != null && channelBot.getProperties() != null) {
			historyTime *= Integer.parseInt(channelBot.getProperties().getProperty("config.channels.history"));
		}
	...
	public boolean join(String username) {
		...
		User permUser = this.channelBot.getUserService().getUser(username);
		if (permUser != null) {
	...
	public boolean kick(String moderator, String username) {
		...
		User user = this.channelBot.getUserService().getUser(username);
		if (user != null && user.getInChannels().contains(getID())) {
			user.getInChannels().remove(getID());
			try {
				this.channelBot.getDatabaseProviderRepository().getChannelUserProvider().deleteChannelUser(this, user);
			} catch (SQLException e) {
				ChannelBot.logError(e);
			}

			tell("", username + " has been removed from the channel by " + moderator + ".");
			this.channelBot.qtell(username,
	 ...
	 * Leaves a channel.
	 *
	 * @param username Username who is leaving the channel
	 * @param silent   When silent=true, the channel is not notified when a player
	 *                 leaves the channel.
	 */
	public void leave(String username, boolean silent) {
		...
		if (permUser != null) {
			...
			try {
				this.channelBot.getDatabaseProviderRepository().getChannelUserProvider().deleteChannelUser(this, permUser);
			} catch (SQLException e) {
				ChannelBot.logError(e);
			}
		}
	...
	public void logTell(final String message) {
		Properties props = this.channelBot.getProperties();
		...
		try {
			...
			ChannelBot.logError(e);
			this.channelBot.qtell(ChannelBot.getChannelBotAdministratorUsername(), "logTell() Failed.");
		}
	}
	 ...
	 *
	 * @param username Username of the player sending the tell.
	 * @param message  Message that the username is sending.
	 */
	public void tell(String username, String message) {
		...
		String usernameTagsRemoved = ServerConnection.stripUserTags(username);

		// send the message to each of the users
		for (int i = 0; i < members.size(); i++) {
			String mess = message;
			String name = members.get(i);
			User user = this.channelBot.getUserService().getUser(name);
			...
			if ((user.getName().equals(usernameTagsRemoved) && user.isEcho())
				...
				if (!user.isShowSwearWords()) {
					mess = this.channelBot.getLanguageFilterService().filterLanguage(mess);
				}
				this.channelBot.qtell(user.getName(), mess);
			}
		}

		User usr = this.channelBot.getUserService().getUser(usernameTagsRemoved);
	...
	public String buildTellHistory(User user) {
		...
		if (size == 0) {
			...
			for (int i = 0; i < size; i++) {
				...
				if (!user.isShowSwearWords()) {
					message = this.channelBot.getLanguageFilterService().filterLanguage(message);
				}
	...
	public void setChannelBot(final ChannelBot channelBot) {

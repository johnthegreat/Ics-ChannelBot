...
public class ChannelProvider {
	...
	public ChannelProvider(final Connection connection) {
	...
	public List<Channel> getChannels() throws SQLException {
		...
		try (final PreparedStatement statement = connection.prepareStatement("SELECT * FROM channel ORDER BY num ASC")) {
			statement.execute();
			final ResultSet resultSet = statement.getResultSet();
			while (resultSet.next()) {
				final Channel channel = createChannelFromResultSet(resultSet);
	...
	public Channel getChannelById(final int channelId) throws SQLException {
		try (final PreparedStatement preparedStatement = connection.prepareStatement("SELECT * FROM channel WHERE num = ?")) {
			preparedStatement.setInt(1, channelId);
			if (preparedStatement.execute()) {
				final ResultSet resultSet = preparedStatement.getResultSet();
				if (resultSet.next()) {
					return createChannelFromResultSet(resultSet);
				}
			}
		}
		return null;
	...
	protected void populateChannelUsersFromDb(Channel channel) throws SQLException {
		try (final PreparedStatement preparedStatement = connection.prepareStatement(
			...
			while (resultSet.next()) {
				...
				final String moderatorLevel = resultSet.getString("moderator");
	...
	public void createOrUpdateChannel(final Channel channel) throws SQLException {
		try (final PreparedStatement preparedStatement = connection.prepareStatement(
	...
	public void updateChannel(final Channel channel) throws SQLException {
		try (final PreparedStatement preparedStatement = connection.prepareStatement(
	...
	public void deleteChannel(int id) throws SQLException {
		try (final PreparedStatement statement = connection.prepareStatement("DELETE FROM channel WHERE num = ?")) {

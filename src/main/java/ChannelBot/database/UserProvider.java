...
public class UserProvider {
	...
	public List<User> getUsers() throws SQLException {
		...
		try (final PreparedStatement statement = connection.prepareStatement("SELECT username FROM user ORDER BY username ASC")) {
			statement.execute();
	...
	public void createOrUpdateUser(User user) throws SQLException {
		try (PreparedStatement statement = connection.prepareStatement(
			...
			statement.setString(7, TimeZoneUtils.getAbbreviation(user.getTimeZone() != null ? user.getTimeZone() : TimeZone.getTimeZone("UTC")));
			statement.execute();
		}
	}
